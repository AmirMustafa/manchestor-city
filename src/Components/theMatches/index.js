...
class TheMatches extends Component {
  ...
  // filtering data - all, played and not played plus resetting right filter
  showPlayed = played => {
    ...
    this.setState({
      ...
      playedFilter: played, // Reset 2nd filter
      resultFilter: "All"
    });
  };

  // filtering data - W, L, D, All
  showResult = result => {
    const list = this.state.matches.filter(match => {
    ...
    this.setState({
      ...
      playedFilter: "All",
      resultFilter: result // Reset 1st filter
    });
  };
  render() {
